...
public class TrainerVideoAlbumSerializer extends JsonSerializer<TrainerVideoAlbum> {

    @Override
    public void serialize(TrainerVideoAlbum album, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) throws IOException {
        if (album instanceof HibernateProxy) {
            // Unwrap the proxy to the actual entity
            album = (TrainerVideoAlbum) ((HibernateProxy) album).getHibernateLazyInitializer().getImplementation();
        }
