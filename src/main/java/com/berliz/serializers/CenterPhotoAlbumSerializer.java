...
public class CenterPhotoAlbumSerializer extends JsonSerializer<CenterPhotoAlbum> {

    @Override
    public void serialize(CenterPhotoAlbum album, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) throws IOException {
        if (album instanceof HibernateProxy) {
            // Unwrap the proxy to the actual entity
            album = (CenterPhotoAlbum) ((HibernateProxy) album).getHibernateLazyInitializer().getImplementation();
        }
    ...
    private byte[] loadPhotoFromFolder(String photoName) throws IOException {
        ...
        String photoFolderPath = BerlizConstants.CENTER_PHOTO_ALBUM_LOCATION;
