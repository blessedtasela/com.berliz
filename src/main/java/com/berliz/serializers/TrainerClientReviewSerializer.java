...
import com.berliz.models.TrainerClientReview;
...
public class TrainerClientReviewSerializer extends JsonSerializer<TrainerClientReview> {

    @Override
    public void serialize(TrainerClientReview clientReview, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) throws IOException {
        if (clientReview instanceof HibernateProxy) {
            // Unwrap the proxy to the actual entity
            clientReview = (TrainerClientReview) ((HibernateProxy) clientReview).getHibernateLazyInitializer().getImplementation();
        }
