...
public class TrainerServiceImplement implements TrainerService {
    ...
    public ResponseEntity<String> addTrainerPhotoAlbum(PhotoAlbum photoAlbum) throws JsonProcessingException {
        try {
            log.info("Inside addTrainerPhotoAlbum {}", photoAlbum);

            boolean isValid = photoAlbum != null;
            log.info("Is request valid? {}", isValid);
            ...
            if (jwtFilter.isAdmin()) {
                if (photoAlbum.getTrainerId() == null) {
                    return BerlizUtilities.buildResponse(HttpStatus.BAD_REQUEST, "Admin must provide trainerId");
                }

                trainer = trainerRepo.findByTrainerId(photoAlbum.getTrainerId());
            }
            ...
            getTrainerPhotoAlbumFromMap(photoAlbum, trainer);
            String responseMessage = jwtFilter.isAdmin() ?
    ...
    public ResponseEntity<String> updateTrainerPhotoAlbum(PhotoAlbum photoAlbum) throws JsonProcessingException {
        try {
            log.info("Inside updateTrainerPhotoAlbum {}", photoAlbum);
            Integer userId = jwtFilter.getCurrentUserId();
            boolean isValid = photoAlbum != null;
            log.info("Is request valid? {}", isValid);
            ...
            if (!isValid || photoAlbum.getId() == null) {
                return BerlizUtilities.buildResponse(HttpStatus.BAD_REQUEST, BerlizConstants.INVALID_DATA);
            }

            TrainerPhotoAlbum existingPhotoAlbum = trainerPhotoAlbumRepo.findById(photoAlbum.getId()).orElse(null);
            ...
            Files.write(path, photoAlbum.getPhoto().getBytes());

            // Update photo details in the database
            existingPhotoAlbum.setComment(photoAlbum.getComment());
    ...
    public ResponseEntity<String> addTrainerBenefit(Map<String, String> requestMap) throws JsonProcessingException {
        try {
            ...
            String responseMessage = jwtFilter.isAdmin() ?
                    trainer.getName() + "!, trainer benefit have successfully been added to their list" :
    ...
    @Override
    public ResponseEntity<String> addTrainerIntroduction(@ModelAttribute Introduction introduction) throws JsonProcessingException {
        try {
            log.info("Inside addTrainerIntroduction {}", introduction);
            boolean isValid = introduction != null;
            log.info("Is request valid? {}", isValid);
            ...
            Trainer trainer = trainerRepo.findByUserId(jwtFilter.getCurrentUserId());
            if (jwtFilter.isAdmin()) {
                if (introduction.getTrainerId() == null) {
                    return BerlizUtilities.buildResponse(HttpStatus.BAD_REQUEST, "Admin must provide trainerId");
                }

                trainer = trainerRepo.findByTrainerId(introduction.getTrainerId());
            }
            ...
            getTrainerIntroductionFromMap(introduction, trainer);
            String responseMessage = jwtFilter.isAdmin() ?
                    trainer.getName() + "!, trainer introduction have successfully been added to their list" :
    ...
    public ResponseEntity<String> updateTrainerIntroduction(Introduction introduction) throws JsonProcessingException {
        try {
            log.info("Inside updateTrainerIntroduction {}", introduction);
            Integer userId = jwtFilter.getCurrentUserId();
            boolean isValid = introduction != null;
            log.info("Is request valid? {}", isValid);
            ...
            if (!isValid || introduction.getId() == null) {
                return BerlizUtilities.buildResponse(HttpStatus.BAD_REQUEST, BerlizConstants.INVALID_DATA);
            }

            TrainerIntroduction trainerIntroduction = trainerIntroductionRepo.findById(introduction.getId()).orElse(null);
            if (trainerIntroduction == null) {
                return BerlizUtilities.buildResponse(HttpStatus.NOT_FOUND, "Trainer introduction not found in db");
            }
            ...
            if (!jwtFilter.isAdmin() && !trainerIntroduction.getTrainer().getId().equals(jwtFilter.getCurrentUserId())) {
                return BerlizUtilities.buildResponse(HttpStatus.UNAUTHORIZED, BerlizConstants.UNAUTHORIZED_REQUEST);
            }

            Trainer trainer = trainerIntroduction.getTrainer();
            byte[] photo = introduction.getCoverPhoto().getBytes();
            trainerIntroduction.setCoverPhoto(photo);
            trainerIntroduction.setIntroduction(introduction.getIntroduction());
    ...
    public ResponseEntity<String> addTrainerVideoAlbum(VideoAlbum videoAlbum) throws JsonProcessingException {
        try {
            log.info("Inside addTrainerVideoAlbum {}", videoAlbum);

            boolean isValid = videoAlbum != null;
            log.info("Is request valid? {}", isValid);
            ...
            Trainer trainer = trainerRepo.findByUserId(jwtFilter.getCurrentUserId());
            if (jwtFilter.isAdmin()) {
                if (videoAlbum.getTrainerId() == null) {
                    return BerlizUtilities.buildResponse(HttpStatus.BAD_REQUEST, "Admin must provide trainerId");
                }

                trainer = trainerRepo.findByTrainerId(videoAlbum.getTrainerId());
            }
            ...
            getTrainerVideoAlbumFromMap(videoAlbum, trainer);
            String responseMessage = jwtFilter.isAdmin() ?
                    trainer.getName() + "!, video have successfully been added to their album" :
    ...
    public ResponseEntity<String> updateTrainerVideoAlbum(VideoAlbum videoAlbum) throws JsonProcessingException {
        try {
            log.info("Inside updateTrainerVideoAlbum {}", videoAlbum);
            Integer userId = jwtFilter.getCurrentUserId();
            boolean isValid = videoAlbum != null;
            log.info("Is request valid? {}", isValid);
            ...
            if (!isValid || videoAlbum.getId() == null) {
                return BerlizUtilities.buildResponse(HttpStatus.BAD_REQUEST, BerlizConstants.INVALID_DATA);
            }

            TrainerVideoAlbum trainerVideoAlbum = trainerVideoAlbumRepo.findById(videoAlbum.getId()).orElse(null);
            ...
            Files.write(path, videoAlbum.getVideo().getBytes());

            // Update video details in the database
            trainerVideoAlbum.setComment(videoAlbum.getComment());
    ...
    @Override
    public ResponseEntity<String> addTrainerClientReview(ClientReview clientReview) {
        return null;
    }

    @Override
    public ResponseEntity<String> updateTrainerClientReview(ClientReview clientReview) {
        return null;
    }

    @Override
    public ResponseEntity<String> deleteTrainerClientReview(Integer id) {
        return null;
    }
    ...
    @Override
    public ResponseEntity<String> addTrainerFeatureVideo(FeatureVideo featureVideo) {
        return null;
    }

    @Override
    public ResponseEntity<String> updateTrainerFeatureVideo(FeatureVideo featureVideo) {
        return null;
    }

    @Override
    public ResponseEntity<String> deleteTrainerFeatureVideo(Integer id) {
        return null;
    }
    ...
    private void getTrainerPricingFromMap(Map<String, String> requestMap, Trainer trainer) throws IOException {
    ...
    private void getTrainerPhotoAlbumFromMap(PhotoAlbum photoAlbum, Trainer trainer) throws IOException {
        ...
        trainerPhotoAlbum.setComment(photoAlbum.getComment());
    ...
    private void getTrainerIntroductionFromMap(Introduction introduction, Trainer trainer) throws IOException {
    ...
    private void getTrainerVideoAlbumFromMap(VideoAlbum videoAlbum, Trainer trainer) throws IOException {
